...
let rec codegen_base b scope ind =
  match b with
  | Ident (_, t) ->
      ("bound " ^ try List.assoc t scope.binds with Not_found -> mangle t)
      |> paren |> gen ind
  ...
  | Tuple ex ->
      "tuple "
      ^ join_on " " (List.map (fun x -> codegen_expr x scope (ind + 1)) ex)
      |> paren |> gen ind
...
and codegen_expr ex scope ind =
  ...
  | FCall (_, f, x) ->
      "fcall " ^ codegen_expr f scope (ind + 1) ^ codegen_expr x scope (ind + 1)
      |> paren |> gen ind
  | AnnotLet (_, l, _, e1, e2) | LetIn (_, l, e1, e2) ->
      ...
      |> paren |> gen ind
  | Join (_, e1, e2) ->
      "seq " ^ codegen_expr e1 scope (ind + 1) ^ codegen_expr e2 scope (ind + 1)
      |> paren |> gen ind
  ...
  | Lam (_, l, e) | AnnotLam (_, l, _, e) ->
      "lam " ^ mangle l ^ codegen_expr e scope (ind + 1) |> paren |> gen ind
  | TupAccess (_, e, i) ->
      "tupaccess " ^ string_of_int i ^ codegen_expr e scope (ind + 1)
      |> paren |> gen ind

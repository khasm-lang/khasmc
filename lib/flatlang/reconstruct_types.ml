...
(* we need to ensure names are bound properly
   ...
   *)
let rec do_names externs nm_map expr =
  let open List in
  let go = reconstruct_e externs nm_map in
  match expr with
  | Expr (dat, Let nm, [ a; _ ]) | Expr (dat, IfLet nm, [ a; _; _ ])
    ->
      go a;
      NameMap.add nm (get_typ a) nm_map
  | Expr (dat, Lambda (nm, input), _) -> NameMap.add nm input nm_map
  | Expr (dat, Unpack (typs, nms), _) ->
      NameMap.add_seq (List.to_seq @@ List.combine nms typs) nm_map
  | _ -> nm_map

and reconstruct_e (externs : (string, typ) Hashtbl.t)
    (nm_map : 'a NameMap.t) (expr : expr) : unit =
  let open List in
  let nm_map = do_names externs nm_map expr in
  let go = reconstruct_e externs nm_map in
  let map' f x = ignore @@ map f x in
  map' go (get_children expr);
  match expr with
  | Expr (dat, tag, children) when dat.typ <> TyUnknown -> ()
  | Expr (dat, Extern nm, _) -> dat.typ <- Hashtbl.find externs nm
  | Expr (dat, Named (`Local, nm), _) -> begin
      match NameMap.find_opt nm nm_map with
      | Some ty -> dat.typ <- ty
      | None ->
  ...
  | Expr (dat, Funccall, ts) -> begin
      match get_typ @@ hd ts with
      | TyArrow (l, r) -> dat.typ <- r
      | x ->
          print_endline "bad function type: ";
          print_endline (show_typ x);
          print_endline "kids:";
          List.iter
            (fun x -> print_endline (show_expr x))
            (get_children expr);
          failwith "bad"
    end
  | Expr (dat, Unpack (_, _), _) -> dat.typ <- TyTuple []
  | Expr (dat, Let _, [ _; bd ]) | Expr (dat, IfLet _, [ _; _; bd ])
  | Expr (dat, IfConst _, [_; _; bd])
    ->
      dat.typ <- get_typ bd
  | Expr (dat, Seq, [ _; b ]) -> dat.typ <- get_typ b
  | Expr (dat, Modify _, _) -> dat.typ <- TyTuple []
  | Expr (dat, Fail _, _) -> dat.typ <- TyBase `Bottom
  | Expr (dat, _, _) -> (
...
let reconstruct top =
  IR.process_in_definitions'
    (fun def x ->
      let map = NameMap.of_list def.args in
      reconstruct_e top.externs map x;
      x)
    top

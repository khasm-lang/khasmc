...
let rec find_naive_definition
    (ctx : ctx) (path : path) : (path, 'a) result =
  match path with
  | End -> err (`Invalid_Argument "Got End where full path was expected")
  | Base n -> (
      match
        full_matches
          ctx.definitions (InMod (ctx.current, Base n)) with
...
let handle_opens (ctx : ctx) : ctx =
  ...
  let new_defs =
    List.fold_left
      (fun acc nm -> remove_one nm acc) ctx.definitions ctx.open'
  in
...
let ensure_includes_correct ctx path =
  ...
  | _ -> (
      ...
      | [] ->
        err
          (`Bad_Include ("No such path wrt includes: " ^ show_path path))
      | _ -> ok path)
...
let handle_file (ctx: ctx) (file: file): file =
  let collect_names =
    ...
    |> List.map (fun x -> (x, x))
  in
  let ctx = {ctx with definitions = collect_names @ ctx.definitions} in
  let defs = List.map (function
      ...
      | Trait (id, trait) ->
        Trait(id, handle_trait ctx trait)
      | Impl (id, impl) ->
        Impl (id, handle_impl ctx impl)
    ) file.toplevel
  in
  { file with
    toplevel = defs
...
let handle_files files =
  ...
  *)
  let ctx =
    { definitions = []; included = []; open' = []; current = "" } in
